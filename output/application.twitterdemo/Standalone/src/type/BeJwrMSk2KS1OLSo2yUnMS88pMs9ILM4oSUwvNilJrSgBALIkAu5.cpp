...
#include "BeJwrMSk2KS1OLSo2yUnMS88pMs9ILM4oSUwvNilJrSgBALIkAu5.h"
#include <sstream>
...
{
    instantiators_.insert(std::make_pair("tuple<rstring user,rstring lang,list<ustring> hashtag,rstring text>",&initer));
...
#define MY_OFFSETOF(member, base) \
    ...
    {
        std::string s("user");
        TypeOffset t(MY_OFFSETOF(user_, tm), 
    ...
    {
        std::string s("lang");
        TypeOffset t(MY_OFFSETOF(lang_, tm), 
                     Meta::Type::typeOf<SPL::rstring >(), 
                     &typeid(SPL::rstring));
        addMapping(*tm, t, s, 1);
    }
    ...
    {
        std::string s("hashtag");
        TypeOffset t(MY_OFFSETOF(hashtag_, tm), 
                     Meta::Type::typeOf<SPL::list<SPL::ustring > >(), 
                     &typeid(SPL::list<SPL::ustring >));
        addMapping(*tm, t, s, 2);
    }
...
void SELF::deserialize(std::istream & istr, bool withSuffix)
{
   ...
   if (s != "user") { istr.setstate(std::ios_base::failbit); return; }
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   if (withSuffix)
     SPL::deserializeWithSuffix(istr, user_);
   else
     istr >> user_;
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
   ...
   if (!readAttributeIdentifier(istr, s)) { return; }
   if (s != "lang") { istr.setstate(std::ios_base::failbit); return; }
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   if (withSuffix)
     SPL::deserializeWithSuffix(istr, lang_);
   else
     istr >> lang_;
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
   ...
   if (!readAttributeIdentifier(istr, s)) { return; }
   if (s != "hashtag") { istr.setstate(std::ios_base::failbit); return; }
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   if (withSuffix)
     SPL::deserializeWithSuffix(istr, hashtag_);
   else
     istr >> hashtag_;
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
...
void SELF::deserializeWithNanAndInfs(std::istream & istr, bool withSuffix)
{
   ...
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   SPL::deserializeWithNanAndInfs(istr, user_, withSuffix);
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
   if (c != ',') { istr.setstate(std::ios_base::failbit); return; }
   ...
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   SPL::deserializeWithNanAndInfs(istr, lang_, withSuffix);
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
   if (c != ',') { istr.setstate(std::ios_base::failbit); return; }
   ...
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   SPL::deserializeWithNanAndInfs(istr, hashtag_, withSuffix);
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
   if (c != ',') { istr.setstate(std::ios_base::failbit); return; }
...
{
    ostr << '{';
    SPL::serializeWithPrecision(ostr << "user=", get_user()) << ",";
    SPL::serializeWithPrecision(ostr << "lang=", get_lang()) << ",";
    SPL::serializeWithPrecision(ostr << "hashtag=", get_hashtag()) << ",";
