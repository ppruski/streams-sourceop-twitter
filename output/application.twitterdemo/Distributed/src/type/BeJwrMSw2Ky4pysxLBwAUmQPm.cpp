...
#include "BeJwrMSw2Ky4pysxLBwAUmQPm.h"
#include <sstream>
...
{
    instantiators_.insert(std::make_pair("tuple<rstring string>",&initer));
...
#define MY_OFFSETOF(member, base) \
    ...
    {
        std::string s("string");
        TypeOffset t(MY_OFFSETOF(string_, tm), 
...
void SELF::deserialize(std::istream & istr, bool withSuffix)
{
   ...
   if (s != "string") { istr.setstate(std::ios_base::failbit); return; }
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   if (withSuffix)
     SPL::deserializeWithSuffix(istr, string_);
   else
     istr >> string_;
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
...
void SELF::deserializeWithNanAndInfs(std::istream & istr, bool withSuffix)
{
   ...
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   SPL::deserializeWithNanAndInfs(istr, string_, withSuffix);
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
...
{
    ostr << '{';
    SPL::serializeWithPrecision(ostr << "string=", get_string()) ;
    ostr << '}';
}
