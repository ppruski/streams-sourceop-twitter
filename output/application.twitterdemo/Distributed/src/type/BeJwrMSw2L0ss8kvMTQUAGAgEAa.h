...
#ifndef BEJWRMSW2L0SS8KVMTQUAGAGEAA_H_
#define BEJWRMSW2L0SS8KVMTQUAGAGEAA_H_
...
public:
    ...
    typedef SPL::rstring varName_type;

    enum { num_attributes = 1 } ;
    
    SELF() : Tuple() {}
    SELF(const Self & ot) : Tuple(), varName_(ot.varName_) 
      { constructPayload(ot); }
    SELF(const varName_type & _varName) : Tuple(), varName_(_varName) { }
    ...
    varName_type & get_varName() { return varName_; }
    const varName_type & get_varName() const { return varName_; }
    void set_varName(const varName_type & _varName) { varName_ = _varName; }
    ...
    {
        size_t s = 17;
        s = 37 * s + std::tr1::hash<varName_type >()(varName_);
        return s;
    }
    ...
    {
        size_t size = 0;
           size += varName_.getSerializedSize();
...
#undef SELF
#endif // BEJWRMSW2L0SS8KVMTQUAGAGEAA_H_ 
