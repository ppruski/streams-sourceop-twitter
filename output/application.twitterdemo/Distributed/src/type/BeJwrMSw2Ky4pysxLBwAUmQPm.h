...
#ifndef BEJWRMSW2KY4PYSXLBWAUMQPM_H_
#define BEJWRMSW2KY4PYSXLBWAUMQPM_H_
...
#define SELF BeJwrMSw2Ky4pysxLBwAUmQPm
...
public:
    ...
    typedef SPL::rstring string_type;

    enum { num_attributes = 1 } ;
    
    SELF() : Tuple() {}
    SELF(const Self & ot) : Tuple(), string_(ot.string_) 
      { constructPayload(ot); }
    SELF(const string_type & _string) : Tuple(), string_(_string) { }
    ...
    string_type & get_string() { return string_; }
    const string_type & get_string() const { return string_; }
    void set_string(const string_type & _string) { string_ = _string; }
    ...
    {
        size_t s = 17;
        s = 37 * s + std::tr1::hash<string_type >()(string_);
        return s;
    }
    ...
    {
        size_t size = 0;
           size += string_.getSerializedSize();
...
#undef SELF
#endif // BEJWRMSW2KY4PYSXLBWAUMQPM_H_ 
